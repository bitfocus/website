...
Since API 2.0, you can customise the sort order of the actions by setting the `sortName` property on an action definition. When this is set, it will be used instead of the `name` when sorting the action definitions alphabetically.
...
Whenever the options of an action on a button is changed, unsubscribe will be called, followed by subscribe.

It is also possible to force either unsubscribe or subscribe to be called for every action, by calling `this.subscribeActions()` or `this.unsubscribeActions()`. Both functions accept `actionIds` parameters, to only run on a certain feedback type (eg `this.unsubsunsubscribeActionscribeFeedbacks('set_source', 'set_source2')`).
