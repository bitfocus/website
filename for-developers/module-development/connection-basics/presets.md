...
A single preset can also offer several [alternatives](#preset-alternatives) (`type: "alternatives"`), letting the host pick the best variant it supports — for example a layered version with a simple fallback.
...
A `layered` preset is a new preset type in [API 2.1](../api-changes/v2.1.md). Instead of a single flat `style`, it draws its button from a stack of [graphics elements](./graphics-elements.md) — text, images, boxes, lines, circles, gauges and groups, plus any [composite elements](./composite-elements.md) your module defines.
...
presets[`play`] = {
  ...
  variants: [
    ...
    {
      // Fallback: a simple version for hosts that can't render the layered one
      type: 'simple',
      name: `Play`,
...
Each entry in `variants` is an ordinary preset definition (`simple` or `layered`); alternatives cannot be nested inside alternatives. The preset is still keyed by a single id, so your [structure](#preset-structure) references it exactly as it would a plain preset.

See the autogenerated docs for [`CompanionPresetAlternatives`](https://bitfocus.github.io/companion-module-base/interfaces/CompanionPresetAlternatives.html).
