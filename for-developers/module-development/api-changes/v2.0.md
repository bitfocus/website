...
- For a `number` field, the value will be coerced to a number. If `clampValues: true` is set, it will be clamped to the `min` and `max` you provide, otherwise it will fail validation. If `asInteger: true` is set, it will be rounded to the nearest integer value, before checking min/max.
...
When a field's value can come from an expression, you cannot reference it from another field in an `isVisibleExpression`. This means any action or feedback field which does not have `disableAutoExpression: true` cannot be referenced.

We disallow this because it can lead to fields being hidden when they are actually needed later as the computed expression value changes, which would both confuse users and prevent them from editing required fields.
